/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { StudentWalletWhereInput } from "./StudentWalletWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { StudentWalletOrderByInput } from "./StudentWalletOrderByInput";

@ArgsType()
class StudentWalletFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => StudentWalletWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => StudentWalletWhereInput, { nullable: true })
  @Type(() => StudentWalletWhereInput)
  where?: StudentWalletWhereInput;

  @ApiProperty({
    required: false,
    type: [StudentWalletOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [StudentWalletOrderByInput], { nullable: true })
  @Type(() => StudentWalletOrderByInput)
  orderBy?: Array<StudentWalletOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { StudentWalletFindManyArgs as StudentWalletFindManyArgs };
